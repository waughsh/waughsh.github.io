...
4. Numbers should be embedded jointly, by type/category. For example "$120" should be embedded differently than "20m". When numbers are embedded by types, then we can define a distance between the vectors to be proportional to the true numerical value (scalar). So each type of number will be embedded jointly, but each single instance of number will be a scalar (of sort) or distance penalty of the other numbers.
